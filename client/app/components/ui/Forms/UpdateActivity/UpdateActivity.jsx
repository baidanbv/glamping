...
const UpdateActivity = ({ activityId, onSuccess, token }) => {
  ...
  const onSubmit = async (data) => {
    ...
    await activitiesAPI.updateActivityById(updatedData, token);
    closeUpdatePopup();
    onSuccess()
  };
  ...
  useEffect(() => {
    const fetchActivity = async () => {
      setLoading(true);
      const activityData = await activitiesAPI.fetchActivityById(activityId);
      reset(activityData); 
      setLoading(false);
  ...
  return (
    <>
      <h2 className="text-3xl font-semibold mb-6 text-center">Update activity</h2>
